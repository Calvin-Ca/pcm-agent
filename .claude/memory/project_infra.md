...
> **连通性验证**：172.19.3.136 → 192.168.0.94:3306 `nc` 超时（网络不通，疑为内网路由/防火墙问题）；172.19.3.136 → 172.19.2.170:3306 `nc` 成功。若生产数据库恢复不了，可将 ai-service 的 `MYSQL_HOST` / `SQL_AGENT_DB_HOST` 切到 `172.19.2.170`。
