...
## 基础设施状态（2026-04-24 08:10 UTC+8）

| 组件 | 地址 | 状态 | 影响 |
|------|------|------|------|
| 116 应用服务器 | 116.205.174.57 | ❌ 网络中断（ping 100% loss） | SpringBoot + nginx + 公网入口全部不可达；SSH 超时 |
| MySQL 数据库 | 192.168.0.94 | ❌ 网络中断（ping 100% loss） | ai-service 每个请求 MySQL 连接超时 15s+，导致响应极慢 |
| 172 GPU 服务器 | 172.19.3.136 | ✅ 正常 | ai-service + vLLM 运行正常 |
| vLLM | 172.19.3.136:8099 | ✅ 正常 | 模型列表可访问 |
| 反向隧道 9901 | 172→116 | ❌ 中断 | autossh 进程在运行但底层 SSH 连接已断 |

> **结论**：当前 116 服务器和 MySQL 数据库同时网络中断，生产链路完全不可用。E2E 全链路测试必须等 116 和数据库恢复后才能执行。
...
- [x] 新 JWT token 已收到（有效至 2026-04-25 07:55 UTC+8）
- [ ] E2E 全链路复测通过（阻塞：116 服务器 + MySQL 数据库网络中断，待恢复后复测）
